...
- `lib/learning-v2/experiments.ts`：FNV-1a 确定性账户分组（加权变体）、实验注册表（当前无在跑实验）、未注册 key 回退 null 防误扩散
- 曝光上报复用 engagement 链路（新增 experiment_exposed 事件类型）
- 测试：TS 276/276（+5：确定性/加权/未注册/分布均匀）、pytest 111/111、质量门 4/4
