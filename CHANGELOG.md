...
**问题**
- TOC 生成链路里，节点 `index` 可能从 `1` 开始返回
- 课程树点击传的是节点 `index`，而学习内容读取和 lesson 落库大量按数组下标处理
- 结果会出现“点击第 x 节，进入后看到第 x+1 节内容”的错位

**修复**
- `normalizeCourseBlueprint` 统一把课程节点索引归一为内部 0-based，保证存储、路由、预生成、lesson 读取都对齐
- 新增回归测试，覆盖“1-based TOC 输入会被规范成 0-based，且第一节内容仍能正确挂载”
- Python Agent 的 TOC prompt 仅把输出示例中的节点序号改为从 `0` 开始，避免继续诱导模型返回 1-based 序号
- 展示层仍保留用户习惯的“第 1 节 / 第 2 节”文案，不把内部 `0` 暴露给用户

### ✨ 优化：澄清页流式消息升级为单主卡交互

**体验问题**
- 一次 AI 回复会拆成“思考卡 + 正文卡 + 问题卡/学习计划卡”多张卡片，视觉上比较散
- 只有思考、还没生成正文时，下方还会留出一块空白区域，观感不自然

**优化**
- `RichStreamingMessage` 改为单条流式消息只渲染一张主卡片
- 思考区域放在主卡片顶部：思考时默认展开、内容流式出现；思考结束后自动折叠，用户仍可手动展开回看
- 题目卡片和学习计划卡片新增嵌入模式，在主卡片内部渲染，不再额外包一层消息气泡
- 新增布局回归测试，确保“只有思考时不留空白区”“开始生成结构化内容后才出现主内容区”

### 🎨 打磨：澄清页主卡片的视觉主次关系

**优化**
- 为思考区增加展示状态模式：只有思考时更突出，结果出现后自动退为更轻的次级区域
- 结果区在接管主内容时增加轻微的进入动效，减少“突然切换”的生硬感
- 调整思考 header、正文区背景和文字层级，让“当前重点是结果内容”更明确
- 新增展示状态测试，锁定“thinking-only / secondary-collapsed”两种关键模式

### 🐛 修复：`<think>` 原文不再泄漏到澄清页正文区

**问题**
- 后端会把包含 `<think>...</think>` 的原始流式内容直接发送给前端
- 前端此前既在思考区展示了思考内容，又把原始 `<think>` 内容当正文继续渲染，造成重复显示

**修复**
- 前端新增流式 `<think>` 解析逻辑，思考内容和可见正文都从同一份原始流中派生
- 未闭合的 `<think>` 片段只进入思考区，不再在主内容区留下可见文本
- 新增回归测试，覆盖完整和未闭合 `<think>` 两种情况

### 🎨 优化：澄清页思考区在 `</think>` 后立即收起，并重做主卡片视觉

**优化**
- 新增思考状态判定逻辑，`</think>` 一出现就结束思考态，不再等整段回复结束才折叠
- “思考中”头部改成更一体化的内嵌层样式，弱化割裂的按钮感
- 去掉思考区与结果区之间生硬的分割线，改为统一外层卡片 + 内嵌思考层 + 主内容层的结构
- 新增状态测试，锁定“think 未闭合继续展开 / think 闭合立即结束”两条关键行为

### 🎨 微调：澄清页思考区与正文区继续减负

**优化**
- 去掉“思考中”左侧点阵外层白底，避免头部出现突兀白块
- 思考中与思考完成统一使用同一套中性配色，思考区底色改为轻灰米色，不再偏纯白
- 下方题目/学习计划不再额外包一层正文容器，直接在主卡片内容区展示，释放更多可用空间

### 🐛 修复：学习计划卡片不再提前渲染默认“为你定制”

**问题**
- 学习计划流式生成初期，`estimatedLevel` 会先落到默认 `beginner`
- 导致底部“为你定制”块会先渲染出“初级”，随后再跳成真实等级，体验不稳定

**修复**
- 新增 learner positioning 可见性判断，只有在等级不再是默认值，或其他定制信息真实出现后，才渲染“为你定制”块
- 补充回归测试，锁定“默认 beginner 不显示 / 有真实内容后再显示”

### 🎨 优化：澄清页用户消息与发送按钮配色

**优化**
- 用户气泡改为更贴合当前页面的暖橙渐变与柔和阴影，避免和主卡片视觉脱节
- 底部输入区发送按钮同步改成同一套暖橙体系，并调整输入区背景，让整体更协调

### 🐛 修复：澄清对话按块流式渲染，去掉重复卡片

**问题**
- AI 输出 `<quiz>` / `<outline>` 时，前端一边解析流式标签，一边又根据 SSE 结构化事件单独创建卡片
- 导致提问和学习路径会出现两套 UI，且带结构化块时普通文本有时会被吞掉

**修复**
- `contentParser` 改为按内容顺序扫描，支持未闭合 `quiz/outline` 块的渐进解析
- `RichStreamingMessage` 改为按块顺序渲染文本、问题、大纲，标签一出现就以对应样式展示并持续填充内容
- `useStreamChat` 中 `questions` / `confirmation` 等 SSE 事件只负责同步 `sessionId`、`currentBlueprint` 等状态，不再额外创建 `QuestionCard` / `OutlineCard`
- 问题卡片在等待下一轮回复时会自动禁用，避免重复点击

### 🐛 修复：节点学习页卡在加载中、题目失败后内容区变白

**问题**
- 进入课程节点后，cards 已经返回，但页面仍停留在 loading 状态
- `questions` 请求失败时，没有把空题目结果回写到节点数据，导致内容区可能没有可渲染步骤

**修复**
- 新增 `shouldEnterLearningPhase`，统一为“只要 cards 已经可用，就允许进入学习态”
- 新增 `buildNodeContentPatch`，统一生成节点内容补丁，questions 失败时也会稳定回写 `[]`
- `generateNodeContent` 现在会在题目生成失败时保留 cards 并同步写入空题目，页面不再白屏
- `LearnPage` 的加载日志和判断逻辑改为与“cards 先返回、questions 后补齐”的生成策略一致

### 🧹 清理：聊天页移除废弃消息分支与 SSE 噪音

**优化**
- `useChatMessages` 移除已废弃的 `question / outline / loading` 消息类型和对应 helper
- `sseParser` 正确忽略 `data: [DONE]` 与裸 `[DONE]`，控制台不再打印无意义解析失败日志
- 修复 `generateConversationSummary` 对解释偏好的截断，恢复全量测试通过

### 🔎 调试增强：TOC 接口透传下游错误详情

**问题**
- TOC 生成失败时，前端只能看到 `Python Agent error: 500`
- 无法判断是 Python Agent 参数校验失败、内部异常，还是响应格式不匹配

**修复**
- `/api/generate/toc` 现在会读取并透传 Python Agent 的原始错误正文
- `CourseContext` 的错误提取逻辑会优先拼接 `details`，前端弹出的错误信息更完整

### 🐛 修复：澄清环节内容重复输出

**问题**
- AI 每次输出问题或学习计划时，都会显示两份（内容类似但不完全相同）

**原因**
- 流式输出时，前端通过 `content_delta` 接收并解析 HTML 标签（`<quiz>`、`<outline>`）
- 流式完成后，Python Agent 又发送了一次结构化的 `questions`/`confirmation` 事件
- 导致前端渲染了两次

**修复**
- Python Agent 只负责保存到 session，不再重复发送结构化事件
- 流式结束后只发送 `session_created` 事件（包含 sessionId）
- 前端通过流式内容解析一次即可
...
### 学习链路修正
- 修正“从课程目录进入节点学习”的状态机：只要当前节点已有知识卡片，就直接进入学习，不再等待 questions 返回
- 将 questions 生成从 `generateNodeContent` 中拆出，改为进入节点后按需后台补齐，避免卡片已返回却仍停留在 loading
- 进入节点后仅在当前节点可学习时才预热下一个节点知识，减少并发错位导致的下个节点 questions 误报 500
- 课程目录页恢复首节点知识预生成，保证目录生成完成后默认开始准备第一节内容

### 测试补强
- 新增“cards 已就绪即可开学”“questions 空数组视为已补齐”“学习步骤允许先卡片后题目”的测试覆盖
- 新增“预加载只命中紧邻下一节点，不会跳到 x+2”的测试覆盖

### 状态机与展示修正
- 学习页主状态改为跟随当前节点 cards 实时更新，避免“当前节点知识已生成但仍显示课程在生成中”
- 下一个节点预加载改为每次进入节点只触发一次，并严格限制为仅检查 `x+1`
- 澄清环节的学习计划卡片补回“个人情况 / 背景知识 / 已掌握知识 / 为什么这样安排”展示
